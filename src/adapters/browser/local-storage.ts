...
function loadProfileNames(): Record<string, string> {
  ...
  try {
    ...
    return Object.fromEntries(
      Object.entries(parsed).flatMap(([username, displayName]) =>
        typeof displayName === "string" ? [[username, displayName]] : [],
      ),
    );
  } catch {
    return {};
...
function saveProfileNames(profileNames: Record<string, string>) {
...
export async function loginOrCreateLocalUser(username: string, password: string): Promise<LocalLoginResult> {
...
export function saveSession(username: string, token = "") {
...
export function loadProfileName(username: string) {
  if (!username) return "";

  return loadProfileNames()[normalizeUsername(username)] || username;
}

export function saveProfileName(username: string, displayName: string) {
